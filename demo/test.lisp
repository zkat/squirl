...
(defun add-circle (demo x y)
  (let* ((mass (shape-dimension demo))
    ...
    (attach-shape (make-circle radius :elasticity 0.8d0 :friction 0.5d0) body)
    (world-add-body (world demo) body)))
...
(defun add-poly (demo x y)
  (let* ((mass (* 50 (shape-dimension demo)))
         ...
         (body (make-body :mass mass :inertia (moment-for-poly mass verts)
                          :position (vec x y))))
    (attach-shape (make-poly verts :friction 0.2d0 :elasticity 0.3d0) body)
    (world-add-body (world demo) body)))
...
(defun add-car (demo x y)
  (let* ((mass (shape-dimension demo))
    ...
    (attach-shape (make-circle radius :elasticity 0.5d0 :friction 0.5d0) wheel1)
    (attach-shape (make-circle radius :elasticity 0.5d0 :friction 0.5d0) wheel2)
    (attach-shape (make-poly verts :friction 0.5d0 :elasticity 0.5d0) body)
